...
the module for the ovation model in python
...
class FluxEstimator(object):
    ...
    def __init__(self, atype, jtype):
        ...
        #Make a seasonal flux estimator object for each season with nonzero weight
        self.seasonal_flux_estimators = {season:SeasonalFluxEstimator(season, atype, jtype) for season in seasons}



    #example for numba usage 
    #@njit # Set "nopython" mode for best performance, equivalent to @njit
    #def go_fast(self,a): # Function is compiled to machine code when called the first time
    #  trace = 0
    ...
    def get_flux_for_time(self, dt, fileinput, hemi):
        ...
        ** only works for northern hemisphere at the moment
        ...
        #get doy of current date dt which is a datetime object
        ...
        #makes the grid in magnetic coordinates with values 0
        ...
        for season in ['winter','spring','summer','fall']:
            #if the weight for a season is greater 0 in one of the hemispheres,  
            #go on (otherwise the flux from this season is 0)
            if weightsN[season] >0:
                #print(season)
                #get one of the four SeasonalFluxEstimator objects, defined when calling the FluxEstimator class           
                ...
                grid_mlatsN, grid_mltsN, grid_fluxN = flux_estimator.get_gridded_flux(avgsw.ec)
    ...
    def season_weights(self,doy):
        ...
        Returns:
        a numpy structured array with a key for each season.
...
class SeasonalFluxEstimator(object):
    ...
    def get_gridded_flux(self, dF, combined_N_and_S=False, interp_N=True):
        ...
        fluxgridN.fill(0)         #old: fluxgridN.fill(np.nan)
        ...
        #fluxgridS.fill(0)       #old:         #fluxgridS.fill(np.nan)
        #Make grid coordinates
        ...
        for i_mlt in np.arange(self.n_mlt_bins):
            #all magnetic latitudes
            for j_mlat in np.arange(self.n_mlat_bins//2):
